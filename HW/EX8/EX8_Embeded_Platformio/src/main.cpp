...
// C/C++ source code generated on : Thu Jan  2 23:14:39 2025
//
...
void loop() {
  ...
  inputs.start_e = digitalRead(start);
  inputs.dec = digitalRead(dec);
  inputs.inc = digitalRead(inc);
  inputs.t_on = digitalRead(t_on);
  inputs.t_off = !digitalRead(t_on);
  inputs.stop = digitalRead(stop);
  ...
  // Map outputs to Arduino pins
  digitalWrite(off_out, outputs.off_out);
  digitalWrite(a1, outputs.a1);
  digitalWrite(on_out, outputs.on_out);
  digitalWrite(a2, outputs.a2);
